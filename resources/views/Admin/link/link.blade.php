...
<div class="x-nav" >
    ...
    <div class="x-body">
      ...
      <xblock>
        <a href="/admin/link/create" class="layui-btn"><i class="layui-icon"></i>添加</a>

        <form class="layui-form layui-col-md12 x-so" style="width:300px; float:right;" method="get" action="/admin/link">
        {{csrf_field()}}
          <cite>搜索：</cite>
        ...
        @foreach($data as $key=>$value)
          <tr>
            ...
            <td style="width:120px; padding:1px;">&nbsp;&nbsp;
              <img src="{{$value->logo}}" style="height:75px;">
            </td>
              ...
              <a onclick="member_stop(this,'{{ $value->id }}')" href="javascript:;" data-id="{{ $value->status }}" title="停用">
                <i class="layui-icon">&#xe601;</i>
                 ...
                 <i class="layui-icon">&#xe62f;</i>
			      ...
			      @endif
              <a href="/admin/link/{{$value->id}}/edit" title="编辑">
                <i class="layui-icon">&#xe642;</i>
              </a>
              <a onclick="link_del(this,'{{ $value->id }}')" href="javascript:;" title="删除">
                <i class="layui-icon">&#xe640;</i>
              </a>
            </td>
          </tr>
    ...
    <script>
      ...
      function changeSort(obj,id){
         ...
         //发送ajax修改这个排序
         $.post('/link/sort',{'sort':sort,'mark':mark,'id':id,'_token':"{{csrf_token()}}"},function(data){
             if(data.status == 0){
               //修改成功
      ...
      function link_del(obj,id){
          layer.confirm('确认要删除吗？',function(index){
              //发异步删除数据
              $.post('{{ url('admin/link/') }}/'+id,{"_token":"{{csrf_token()}}","_method":"delete","id":id},function(data){
                  if(data.status == 0){
